...
function collectOverrides(){
  const map={}; rowsBox.querySelectorAll('input[type=checkbox]').forEach(i=>{
    ...
    if(i.checked) map[path][k]=true;
  });
  for(const k in map){ const p=map[k]; if(!(p.view||p.create||p.edit||p.delete||p.archive)) delete map[k]; }
  return map;
}
...
async function deleteEmployee(){
  ...
  firstInp.value=lastInp.value=emailInp.value=''; Array.from(rolesSel.options).forEach(o=>o.selected=false);
  renderPermRows({}); await loadEmployees(); msg('Deleted.');
}
...
async function clearOverrides(){
  ...
  renderPermRows({}); msg('Overrides cleared.');
}

// ---------- Save & Send Invite (email link) ----------
async function saveAndInvite(){
  clearMsg(); if(!currentId) return msg('Select an employee first.', false);
  const email = clean(emailInp.value); if(!email) return msg('Enter an email first.', false);

  // 1) Save latest details
  await saveEmployee();

  // 2) Create an invite record (so we know which roles to apply on finish)
  ...
  // keep for "finish" page if browser needs email confirmation
...
(async function init(){
  try{
    if(!window.DF_MENUS || !Array.isArray(window.DF_MENUS.tiles)){ msg('menus.js not loaded.', false); return; }
    await loadRoleOptions(); await loadEmployees(); renderPermRows({});
  ...
  }catch(e){ msg('Init error: ' + (e?.message||e), false); }
})();